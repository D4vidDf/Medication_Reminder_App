...
internal fun HomeScreenContent(
    ...
    val cardWidth = if (isTablet) 300.dp else (screenWidthDp * (2f / 3f))

    // Fixed padding for simplicity, adjust if needed for perfect peeking based on cardWidth
    val pagerHorizontalPadding = 48.dp
                    ...
                    item {
                        Column {
                            ...
                            HorizontalPager(
                                ...
                                modifier = Modifier
                                    .fillMaxWidth()
                                    .height(240.dp), // Increased height for taller cards + pager padding
                                contentPadding = PaddingValues(horizontal = pagerHorizontalPadding),
                                pageSpacing = 16.dp // Increased spacing
                            ) { pageIndex ->
                                ...
                                // Scale and Alpha are still driven by pageOffset
                                val scale = lerp(
                                    start = 0.95f, // Subtle scale for centered item emphasis
                                    stop = 1.0f,
                                ...
                                Box(
                                    modifier = Modifier
                                        .width(cardWidth) // Apply calculated cardWidth
                                        .fillMaxHeight()
                                        .graphicsLayer {
                                            scaleX = scale
                                            scaleY = scale
